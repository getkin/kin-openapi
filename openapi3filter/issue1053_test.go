...
func TestValidateRequestDefaultArrayWithoutExplode(t *testing.T) {
	const spec = `
openapi: 3.0.0
info:
  title: 'Validator'
...
`
	...
	tests := []struct {
		...
		{
			name: "Valid request without type parameters set, using default values",
			args: args{
				url:      "/category",
		...
		{
			name: "Valid request with & type parameters set",
			args: args{
				url:      "/category?type=A",
				expected: []string{"A"},
			},
		...
		{
			name: "Invalid request with 1 type parameters set out of enum",
			args: args{
				url:      "/category?type=X",
				expected: nil,
			},
