...
* `(*openapi3.T).Validate(..)` now rejects documents whose `openapi` version string doesn't parse or names a major version other than 3 (e.g. `4.0.0`, `v3.1`, `3.x`), with the new `openapi3.OpenAPIVersionUnsupportedError` (code `openapi-version-unsupported`). Previously such documents validated silently, with every OpenAPI 3.1+ construct rejected as if they were 3.0.
* `(*openapi3.T).OpenAPIMajorMinor()`, `IsOpenAPI30()`, `IsOpenAPI31OrLater()` and `IsOpenAPI32OrLater()` now parse the version numerically instead of matching a fixed list of known version strings. Minor and patch releases this library predates are recognized (`3.0.99` is 3.0, `3.2.1` is 3.2, `3.10.0` is 3.10 — and sorts after 3.2, not before it). `OpenAPIMajorMinor()` returns `"4.0"` rather than `""` for `4.0.0`, and the `OrLater` predicates return true for any later version, including majors above 3.
