...
func (schema *Schema) validate(ctx context.Context, stack []*Schema) ([]*Schema, error) {
	...
	for _, item := range schema.OneOf {
		v := item.Value
		if v == nil {
			return stack, foundUnresolvedRef(item.Ref, item.Origin)
		}
	...
	if ref := schema.Not; ref != nil {
		v := ref.Value
		if v == nil {
			return stack, foundUnresolvedRef(ref.Ref, ref.Origin)
		}
...
func (schema *Schema) visitNotOperation(settings *schemaValidationSettings, value any) (err error) {
	if ref := schema.Not; ref != nil {
		v := ref.Value
		if v == nil {
			return foundUnresolvedRef(ref.Ref, ref.Origin)
		}
...
func (schema *Schema) visitXOFOperations(settings *schemaValidationSettings, value any) (err error, run bool) {
	...
	if v := schema.OneOf; len(v) > 0 {
		...
		for idx, item := range v {
			v := item.Value
			if v == nil {
				return foundUnresolvedRef(item.Ref, item.Origin), false
			}
...
func (schema *Schema) visitJSONArray(settings *schemaValidationSettings, value []any) error {
	...
	if itemSchemaRef := schema.Items; itemSchemaRef != nil {
		...
		if itemSchema == nil {
			return foundUnresolvedRef(itemSchemaRef.Ref, itemSchemaRef.Origin)
		}
	...
	for _, k := range componentNames(value) {
		...
		if properties != nil {
			...
			if propertyRef != nil {
				p := propertyRef.Value
				if p == nil {
					return foundUnresolvedRef(propertyRef.Ref, propertyRef.Origin)
				}
